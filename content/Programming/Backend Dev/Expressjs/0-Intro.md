...
When we start an Express.js server, it listens on a specified port for incoming HTTP requests. The server is usually run on [localhost](../../Frontend/localhost.md) during development to test the application. Look into [HTTP requests](../HTTP%20requests.md) to learn about the basic requests a server deals with. All data from the HTML to server is usually done through [HTML forms](../../Frontend/HTML/HTML%20forms.md). These HTTP concepts are *fundamental* before moving onto next portion.
