...
Using  `addEventListener` we can attach this listener to an element. When attached, we can use [anonymous functions](anonymous%20functions.md) along with [this](this.md) keyword to make changes to that element directly.  If a specific function is passed to `addEventListener`, then it becomes a [Higher-Order Function](Higher-Order%20Functions.md). These are known as [callback functions](callback%20function.md) and they can take in arguments as well. One of the common argument it takes is the `event` argument.
